// ============================================================
// components/analyze/ResultsPanel.tsx
// Right panel: env data strip, score bars, expandable cards, AI insight
// ============================================================

import React from 'react';
import {
  Thermometer, Droplets, Wind, Eye, Gauge, Users, Mountain,
  Sprout, Building2, Factory, Sun, ChevronDown, ChevronUp,
  AlertTriangle, Zap, ArrowRight, CheckCircle2, Layers,
} from 'lucide-react';
import type { SuitabilityResult, EnvPayload, SuitabilityCategory, AnalyzePhase } from '../../types';
import { Pill, Badge, SectionLabel, ScoreRing, Skeleton } from '../shared/ui';
...
function EnvStrip({ env }: { env: EnvPayload }) {
  ...
  const pills = [
    ...
    { icon: <Gauge size={11} />,       label: 'AQI',     value: `${Math.round(aq.aqi)} · ${aqiLabel(aq.aqi)}`, valueColor: aqiColor(aq.aqi) },
...
function ScoreBars({ scores, expandedCategory, onToggle }: {
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--z-border-subtle)', borderRadius: 'var(--z-radius-lg)', padding: '18px', marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <SectionLabel>SUITABILITY SCORES</SectionLabel>
        <div style={{ display: 'flex', gap: 6 }}>
          {scores.map(s => (
            <div key={s.label} style={{ textAlign: 'center' }}>
              <div style={{ fontFamily: 'var(--z-font-display)', fontSize: 15, fontWeight: 700, color: scoreColor(s.score), lineHeight: 1 }}>{s.score}</div>
              <div style={{ fontFamily: 'var(--z-font-mono)', fontSize: 8, color: 'var(--z-text-faint)', marginTop: 2 }}>{s.label.slice(0,3).toUpperCase()}</div>
            </div>
          ))}
        </div>
      </div>
          ...
          <div key={s.label} style={{ marginBottom: 12 }}>
            ...
            <div style={{ height: 6, background: 'rgba(255,255,255,0.05)', borderRadius: 3, overflow: 'hidden' }}>
              <div style={{
                ...
                animation: 'z-barfill 0.9s var(--z-spring) forwards',
                ['--target-w' as string]: `${s.score}%`,
              }} />
            </div>
          </div>
        );
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 'var(--z-radius-lg)', marginBottom: 8, overflow: 'hidden',
        transition: 'border-color var(--z-duration-base)',
      }}
    >
      {/* Header */}
      <div
        onClick={onToggle}
        style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '13px 16px', cursor: 'pointer' }}
      >
        <div style={{ width: 38, height: 38, borderRadius: 11, background: score.color + '14', border: `1px solid ${score.color}28`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      {/* Expanded body */}
      {expanded && (
        <div style={{ padding: '0 16px 16px', borderTop: `1px solid ${score.color}18`, animation: 'z-fadein 0.2s ease-out' }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontFamily: 'var(--z-font-mono)', fontSize: 9, color: 'var(--z-text-faint)', letterSpacing: '0.1em', marginBottom: 8 }}>KEY FACTORS</div>
            {score.keyFactors.map((f, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 7, fontSize: 11, color: 'var(--z-text-muted)', marginBottom: 5, lineHeight: 1.5 }}>
          ...
          {/* Actions */}
          <div>
            <div style={{ fontFamily: 'var(--z-font-mono)', fontSize: 9, color: 'var(--z-text-faint)', letterSpacing: '0.1em', marginBottom: 8 }}>RECOMMENDED ACTIONS</div>
            {score.actions.map((a, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 7, fontSize: 11, color: 'var(--z-text-secondary)', marginBottom: 6, lineHeight: 1.5 }}>
  ...
  return (
    <div style={{ padding: '4px 0' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
        <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#4ade80', animation: 'z-pulse 1s ease-in-out infinite' }} />
        <span style={{ fontFamily: 'var(--z-font-mono)', fontSize: 10, color: '#4ade80', letterSpacing: '0.1em' }}>
      ...
      <div style={{ marginTop: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
        <Skeleton h="72px" w="100%" delay="0.15s" />
        <Skeleton h="72px" w="100%" delay="0.25s" />
        <Skeleton h="72px" w="100%" delay="0.35s" />
      </div>
    </div>
  );
}
...
function IdleState({ onDemoClick }: { onDemoClick: (name: string, lat: number, lng: number) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', padding: '32px 24px', gap: 20 }}>
      <div style={{ width: 56, height: 56, borderRadius: 16, background: 'rgba(74,222,128,0.08)', border: '1px solid var(--z-border-accent)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Zap size={26} color="#4ade80" strokeWidth={1.5} />
      </div>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontFamily: 'var(--z-font-display)', fontSize: 18, fontWeight: 700, color: 'var(--z-text-primary)', marginBottom: 8, letterSpacing: '-0.02em' }}>Select a location</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, width: '100%' }}>
        {([['Agriculture', Sprout, '#4ade80'], ['Housing', Building2, '#60a5fa'], ['Industry', Factory, '#fb923c'], ['Renewables', Sun, '#fbbf24']] as [string, React.ElementType, string][]).map(([label, Icon, color]) => (
          <div key={label} style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--z-border-subtle)', borderRadius: 'var(--z-radius-md)', padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, justifyContent: 'center' }}>
        {demos.map(d => (
          <button
            ...
            style={{
              padding: '6px 13px', borderRadius: 'var(--z-radius-pill)',
              border: '1px solid var(--z-border-mild)', background: 'transparent',
              color: 'var(--z-text-muted)', cursor: 'pointer',
              fontSize: 11, fontFamily: 'var(--z-font-body)',
              transition: 'all var(--z-duration-fast)',
            }}
            onMouseEnter={e => { const el = e.currentTarget as HTMLButtonElement; el.style.borderColor = 'rgba(74,222,128,0.35)'; el.style.color = '#4ade80'; el.style.background = 'rgba(74,222,128,0.06)'; }}
            onMouseLeave={e => { const el = e.currentTarget as HTMLButtonElement; el.style.borderColor = 'var(--z-border-mild)'; el.style.color = 'var(--z-text-muted)'; el.style.background = 'transparent'; }}
          >
            {d.name}
          </button>
        ))}
...
export function ResultsPanel({ phase, result, envPayload, expandedCategory, onToggleCategory, onDemoClick, error, onRetry }: ResultsPanelProps) {
  if (phase === 'idle') return <IdleState onDemoClick={onDemoClick} />;
  if (phase === 'loading_data' || phase === 'loading_ai') return <div style={{ padding: '20px' }}><LoadingState phase={phase} /></div>;
  if (phase === 'error') return (
    <div style={{ padding: 24, margin: 16, background: 'rgba(248,113,113,0.05)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 'var(--z-radius-lg)', textAlign: 'center' }}>
      ...
      <button onClick={onRetry} style={{ padding: '8px 18px', borderRadius: 'var(--z-radius-md)', border: '1px solid rgba(248,113,113,0.3)', background: 'transparent', color: '#f87171', cursor: 'pointer', fontSize: 12, fontFamily: 'inherit' }}>
        Try Again
      </button>
    </div>
  );
  ...
  return (
    <div style={{ padding: '16px 20px', animation: 'z-fadein 0.4s ease-out' }}>
      {/* Env data */}
      ...
      {/* AI insight */}
      <div style={{ background: 'rgba(74,222,128,0.04)', border: '1px solid rgba(74,222,128,0.14)', borderRadius: 'var(--z-radius-lg)', padding: '16px 18px', marginTop: 8, marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 10 }}>
          <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#4ade80', animation: 'z-pulse 2s ease-in-out infinite' }} />
          <span style={{ fontFamily: 'var(--z-font-mono)', fontSize: 9, color: '#4ade80', letterSpacing: '0.12em' }}>
      ...
      {/* Disclaimer */}
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '10px 14px', background: 'rgba(251,191,36,0.04)', border: '1px solid rgba(251,191,36,0.1)', borderRadius: 'var(--z-radius-md)', fontSize: 10, color: 'var(--z-text-muted)', lineHeight: 1.6, marginBottom: 8 }}>
